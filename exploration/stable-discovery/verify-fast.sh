...
tlc_run_root="$model_root/tlc-runs/$$"
...
declared_source_bridges=$(
  sed -n \
    's#.*load-file "target/exploration/stable-discovery/\([a-z_]*_refinement_bridge\.clj\)".*#\1#p' \
    "$0" |
    sort
)
...
expected_tla_assumption_fingerprint='3680525918:1956'
...
dafny_check_two() {
  "$dafny" verify \
    ...
    "$model_root/WeightedResponseLease.dfy" \
...
dafny_check_three() {
  "$dafny" verify \
    ...
    "$model_root/ReadableWorkIndex.dfy" \
...
source_refinement_check() {
  ...
  (
    ...
    clojure -J-Djava.io.tmpdir="$source_refinement_tmp" -M:dev -e \
      '(do
         (load-file "target/exploration/stable-discovery/randomized_refinement.clj")
         (load-file "target/exploration/stable-discovery/source_refinement_bridge.clj")
         (load-file "target/exploration/stable-discovery/public_schema_refinement_bridge.clj")
         (load-file "target/exploration/stable-discovery/sealed_plan_refinement_bridge.clj")
         (load-file "target/exploration/stable-discovery/cursor_refinement_bridge.clj")
         (load-file "target/exploration/stable-discovery/physical_scheduler_refinement_bridge.clj")
         (load-file "target/exploration/stable-discovery/progress_checkpoint_refinement_bridge.clj")
         ...
         (prn (eacl.exploration.physical-scheduler-refinement-bridge/run-bridge!))
...
tlc_reducer_read_ahead &
tlc_reducer_read_ahead_pid=$!
tlc_service_lifecycle &
tlc_service_lifecycle_pid=$!
tlc_descriptor_coalescing &
tlc_descriptor_coalescing_pid=$!
tlc_progress_checkpoint &
...
tlc_reducer_read_ahead_failed=0
wait "$tlc_reducer_read_ahead_pid" || tlc_reducer_read_ahead_failed=1
tlc_service_lifecycle_failed=0
wait "$tlc_service_lifecycle_pid" || tlc_service_lifecycle_failed=1
tlc_descriptor_coalescing_failed=0
wait "$tlc_descriptor_coalescing_pid" || tlc_descriptor_coalescing_failed=1
...
dafny_obligations=$(awk \
  ...
  "$tlc_run_root/dafny-three.log" \
  "$tlc_run_root/dafny-four.log")
expected_dafny_obligations=536
...
if [ "$tlc_reducer_read_ahead_failed" -ne 0 ] || \
   [ "$tlc_service_lifecycle_failed" -ne 0 ] || \
   [ "$tlc_descriptor_coalescing_failed" -ne 0 ] || \
   [ "$tlc_progress_checkpoint_failed" -ne 0 ] || \
