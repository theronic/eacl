# Relationship storage 7 → 8 in EACL v8

EACL **v8** uses permission storage **8** and Relationship storage **8**.
Storage 8 adds a fifth, nullable qualifier reference to each endpoint tuple.
...
sets `:eacl/storage-version` to 8. Startup checks metadata, physical target
...
explicit bootstrap operations install a completed storage-8 marker. Ordinary
