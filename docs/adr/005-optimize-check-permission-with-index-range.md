...
The `eacl/can?` implementation answers the question, "can <subject> do <permission> to <resource>?" via recursive Datomic Datalog rules.
...
- `lookup-resources` has been refactored to use direct Datomic index access via `datomic.api/index-range` to traverse the index to efficiently enumerate  efficiently query to  still performing poorly due to arrow permissions and pagination offset handling.
