# v6 vs v7.4 reference benchmark
...
| | v6 `f8a1a98` | v7.4 `:cache false` | v7.4 default cache | v7.4 `:live-results?` |
|---|---:|---:|---:|---:|
...
44.39 s measured alongside v7.4 is the same figure under current machine state, and is what the
comparison above uses.

## Reading the numbers

**Uncached, v7.4 is modestly faster.** 1.16× on the full walk, 1.39× on page 1. The engine changed
...
**v7.4 is slower on the last page**: 2.70 ms vs v6's 1.05 ms. In absolute terms both are fast, and
