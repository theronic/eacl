...
(def spicedb-parser
  (insta/parser
    "schema = definition+
     ...
     permission-expr = direct-or-arrow (<'+'> direct-or-arrow)*
...
(defn extract-definitions [parse-tree]
  (->> parse-tree
       ...
       (map (fn [[_ name definition-body]]
              [(second name) {:relations   (extract-relations definition-body)
...
(comment
  ...
  (for [{:as definition
         resource-type :name
         :keys [relations permissions]} *defs]
    (concat
      (for [{:as           relation
             relation-name :name
