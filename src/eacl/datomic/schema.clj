...
(def v4-schema
  [{:db/ident       :resource/type
   ...
   ; can't be unique in Datomic because of nil values, and not all perms are arrows.
   {:db/ident       :eacl.permission/resource-type+arrow-source-relation-name+arrow-target-permission-name+permission-name
    :db/doc         "EACL Permission: Unique identity tuple to enforce uniqueness of [Resource Type + Relation + Permission]."
    :db/valueType   :db.type/tuple
    :db/tupleAttrs  [:eacl.permission/resource-type
                     :eacl.permission/arrow-source-relation-name
                     :eacl.permission/arrow-target-permission-name
                     :eacl.permission/permission-name]
    :db/cardinality :db.cardinality/one}
   ;:db/unique      :db.unique/identity} ; can't be unique because of nils, unless we model arrows w/diff schema.
...
(def v3-schema                                              ; outdated (pre-tuple optimization)
  [{:db/ident       :resource/type
    :db/doc         "Resource Type on Domain Entities. Used by EACL."
    :db/valueType   :db.type/keyword
    :db/cardinality :db.cardinality/one
