...
The v8 adapter SHALL validate the specific seven-slot format identity, not arity or version number alone, and SHALL use only this endpoint-pair layout for
...
- **WHEN** a DataScript database created from the old relationship-entity branch is opened with v8
- **THEN** its old entities are not interpreted as active v8 Relationships
- **AND** startup rejects the incompatible populated store and directs the user to recreate the database or reload Relationships through the v8 API
...
- **THEN** v8 rejects it as incompatible even if a numeric version marker says 9
