...
EACL SHALL expose no scheduled activation through Relationship `valid-from`, transaction metadata, or schedule references in this v8 contract. Unsupported start/schedule qualifiers SHALL be rejected rather than ignored. A successfully stored Relationship SHALL be eligible as soon as a reader selects a database basis containing it, subject to expiry and Caveats.
