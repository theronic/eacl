...
forward and reverse traversal. v8 must preserve the two-datom hot path while
...
- Freeze an eight-component v8 ABI that can support both native validity and
...
- Use a scheduler or garbage collector to make activation or expiration
  correct.
- Implement an automatic, online, offline, or mixed-format v7-to-v8
  relationship migration.
- Preserve arbitrary past valid-time reconstruction beyond each backend's
  configured history retention.
- Add further inline relationship qualifiers after v8; eight components consume
  Datomic's tuple arity budget.

## Decisions

### 1. Use one fixed eight-component v8 endpoint ABI
...
```clojure
:eacl.v8.relationship/subject-type+relation+resource-type+resource+caveat+caveat-context+valid-from+valid-until
:eacl.v8.relationship/resource-type+relation+subject-type+subject+caveat+caveat-context+valid-from+valid-until
```
...
ordinary lookup complexity because v8 permits only one stored relationship per
...
{:db/id caveat-eid
 ...
 :eacl.caveat/profile :spicedb-v8}
```
...
- `:create`: conflict when any stored v8 assertion has the same owner and first four
...
assertions with independent IDs, not duplicate v8 tuples.

### 6. Phase 1 implements fixed storage and native validity

Phase 1 installs and exclusively reads/writes the eight-slot v8 values. Caveat
...
```clojure
;; Attribute :eacl.v8.relationship/expiration
...
perform relation-prefix scans over their v8 forward values. A sparse
...
v8 never reads the v7 endpoint attributes for authorization. Startup rejects:

- any v7 forward or reverse relationship datom;
- mixed v7/v8 relationship data;
...
must rebuild or reseed relationship data into the v8 shape before running v8.
...
- **Eight slots consume Datomic's tuple arity ceiling** → Treat v8 as the final
...
### Phase 1 — v8 storage and native validity
...
These are probes
of the proposed model and current primitives; v8 is not implemented.
...
- **Exclusion failure (high confidence):** a public EACL in-memory check of
  ...
  corrupt negative edge; it is not a claim that a v8 implementation was tested.
...
access comparison; it was not opened as a v8 EACL database. Both layouts had
1,000,000 forward and 1,000,000 reverse datoms. The v8 layout additionally had
...
All identities shared one subject and relation. The first 999,980 v8 identities
...
| v8 first stored page | 11 | 0.024041 ms | 0.022875–0.027916 ms | 20 advanced / 20 returned |
| v8 first effective page | 7 | 111.445917 ms | 109.271958–113.981084 ms | 1,000,000 advanced / 20 returned |
| 1,000 v7 exact raw probes | 11 | 1.817750 ms | 1.697208–2.725416 ms | fixed physical identity |
| 1,000 v8 prefix/group probes | 11 | 4.065000 ms | 3.639708–22.944958 ms | fixed physical identity, up to two candidates |

The v8 effective scan used one `seek-datoms`, checked the selected entity and
...
these warm machine-specific timings. Public v8 latency, cold-cache behavior,
