...
## 1. Freeze the v8 contracts

- [ ] 1.1 Add relationship storage version 8 and bump engine, adapter, cache, subproblem, and cursor ABI identifiers; verify old artifacts and old storage stamps are rejected in contract tests.
- [ ] 1.2 Add shared constants for the two v8 attribute names, value arity eight, endpoint-local identity arity four plus the owning endpoint eid, and qualifier positions; verify every backend imports the shared definitions rather than duplicating slot numbers.
...
- [ ] 1.4 Add startup qualification that rejects any v7 relationship datom or mixed v7/v8 data; verify a populated v7 database fails before serving while a fresh/rebuilt v8 database opens.
- [ ] 1.5 Remove any proposed dual-read, automatic migration, or per-relation fallback path from v8 code and docs; verify instrumentation observes no v7 relationship seek during v8 authorization.
...
- [ ] 2.3 Replace Datalevin relationship schema and write-policy stamp rules for the v8 attributes and verify admitted writes advance only the affected relation generation.
...
- [ ] 2.5 Update clean database bootstrap and storage-version stamping for v8 and verify no v7 attribute is read as an authoritative relationship source.
...
- [ ] 3.6 Update relationship filters, reads, counts, relation-in-use checks, and schema-orphan probes for v8 values; verify future and expired assertions remain visible to stored-data safety checks.
...
- [ ] 4.5 Update object deletion to remove both v8 halves and owned auxiliary values; verify self-relationships, missing endpoints, and one-sided corruption.
...
- [ ] 7.3 Benchmark v7 versus Phase 1 v8 direct checks, negative checks, adjacency, arrows, lookup, count, and pagination; report candidate-stream seeks, pair/ownership/content validation, advanced datoms, and total request work separately.
