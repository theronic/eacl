...
- Put all Caveat components before validity components, while keeping every
  ...
  validity ships.
- Deliver v8 in two implementation phases:
  - **Phase 1 — fixed v8 storage and native validity**: install the eight-slot
...
- Match SpiceDB relationship identity semantics: one stored relationship may
  ...
  qualifiers.
- **BREAKING**: Provide no v7-to-v8 relationship migration or dual-read path.
  A v8 client accepts a fresh/rebuilt relationship store only and fails startup
...
- `converged-relationship-storage`: Replace four-slot v7 endpoint values with
  fixed eight-slot v8 values while retaining two authoritative datoms and one
...
- Requires operators upgrading a populated database to export/re-transact
  Relationships into a fresh v8 database or otherwise rebuild all relationship
  data before running v8.
