...
subproblem values, and restored entries SHALL change for the v8 relationship
...
- **WHEN** a v8 client restores a cache snapshot produced by the old
  relationship ABI
...
- **WHEN** a shared provider returns an authenticated entry carrying an old
  authorization ABI
- **THEN** the v8 client treats it as a miss
