# relationship-storage-upgrade Specification

## Purpose

Defines the explicit, restartable, fail-closed upgrade from four-component v7 Relationship tuples to the single-source five-component v8 qualifier-reference representation.

## Requirements

### Requirement: Client startup requires target Relationship storage
A serving v8-storage client SHALL establish through bounded metadata, physical-schema, migration-state, and legacy-existence checks that its selected database uses Relationship storage version 8 before it permits authorization, Relationship writes, cursor issuance, or cache publication. Current v7 data, mixed data, an incomplete migration, or an incompatible target schema MUST prevent construction. Normal client startup MUST NOT enumerate all v8 Relationships or repeat migration integrity verification.
...
- **WHEN** a client or v7-to-v8 migration is opened against current v6 Relationship entities
...
Each mutable bundled backend SHALL expose a documented `v7-to-v8/migrate!` operation accepting its native connection or writer handle. Client construction MUST NOT implicitly invoke that operation.
...
Before writing storage version 8, migration SHALL prove that no current v7 Relationship datoms remain, every current v8 half has its exact peer, first-four logical identities are unique, the target count and canonical content digest match the durable preflight source certificate, and all affected Relation mutation versions reflect conversion.

#### Scenario: Verification passes
- **WHEN** all final checks pass on one selected target snapshot
- **THEN** migration atomically records complete state and storage version 8
...
- **WHEN** an operator reads the v7-to-v8 guide
...
- **THEN** the guide directs the operator to stop v8 peers and restore/switch to the pre-migration database with the matching old EACL version
