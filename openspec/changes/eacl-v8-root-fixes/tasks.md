...
- [ ] 3.1 Memoize zero-arity `:schema-proof` with a per-adapter-instance delay in `make-adapter`; keep the scoped arity uncached; adapter-double test asserting one computation per instance
...
- [ ] 3.3 Add nil-store short-circuits to the four denotation lookup/resolve fns (key/dependency construction gated behind store presence)
