...
> Archived 2026-08-15 with 16 tasks open. Certification-ledger and bundle work (12.11–12.15, 13.1, 13.6) is tracked as `:unmet-required-obligations` in `formal/verification/manifest.edn` and targeted the generated evaluator that `adopt-stable-discovery-enumeration` replaced; 3.1, 5.7, 12.4, 11.4 were superseded by that change (deadline cut-point, service admission, stable perf gates); 1.3, 2.7, 9.6, 9.7, 12.8 remain partially implemented (see docs/reports/2026-08-15-eacl-stable-engine-audit.md §8) and are candidates for a follow-up change.
...
- [x] 13.11 Qualify matched-v7 latency ratios by exact host and JVM class, including OS, architecture, OS version, processor, memory, heap, JDK, VM implementation, backend/runtime, and measurement method; reject incomplete baseline metadata, report mismatched or incompletely observable current hosts as not applicable with missing fields, and preserve portable correctness and deterministic-work gates.
