...
Each adapter SHALL advertise only the native revision, authoritative-head, and exact-snapshot operations supported by its configured backend and SHALL reject unsupported consistency modes before cache access.
...
#### Scenario: Unknown cache authority still uses native tokens
- **WHEN** a client uses `:coherence-authority :unknown` but its adapter and lifecycle support a native revision operation
- **THEN** EACL may issue and select authenticated native revision tokens while keeping cross-snapshot managed answer reuse disabled
...
Native revision token issuance and validation SHALL NOT require graph-head writes, mutation anchors, mutation-journal retention, mutation fingerprints, or anchor pruning.

#### Scenario: Relationship write returns a token
- **WHEN** an EACL relationship writer commits successfully under managed authority
