...
Opaque pagination cursor encoding/decoding for the Datomic client: loud typed failures, configurable TTL, and schema-change detection. Covers `cursor->token`, `token->cursor`, and cursor plumbing in `eacl.datomic.core` / `eacl.datomic.impl.indexed`.
