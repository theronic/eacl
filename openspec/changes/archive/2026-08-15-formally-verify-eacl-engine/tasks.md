...
> Archived 2026-08-15 with task 13.10 (independent security/formal-methods review) open. That review is an external gate recorded in `formal/verification/manifest.edn` (`:external-certification {:status :unsigned}`) and `formal/verification/external-certifier-procedure.md`; archiving does not close it. The generated kernel is no longer the traversal authority (see `adopt-stable-discovery-enumeration`).
