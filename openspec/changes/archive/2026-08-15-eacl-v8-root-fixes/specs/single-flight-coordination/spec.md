...
### Requirement: Deduplication preserved
Identical concurrent misses on one subproblem key within one lifecycle SHALL result in one computation, except where the wedge-free mechanism's documented fallback applies; joiners SHALL receive the owner's result or the owner's error.

#### Scenario: Identical concurrent misses
- **WHEN** K threads miss on the same key concurrently under normal (non-saturated) conditions
- **THEN** exactly one compute callback runs and all K callers observe its outcome
