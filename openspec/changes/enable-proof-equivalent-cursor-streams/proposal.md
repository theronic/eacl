...
3. Exact fallback at the cursor's own basis is accepted by identity only in the Datomic client (`exact-fallback-decision`); the shared relay requires frame equality there. Identity already binds the cursor to that immutable original basis, so a second frame read cannot strengthen the decision and makes fallback depend unnecessarily on proof-provider availability. `add-authorization-views` deletes the Datomic client, so the rule must move.
