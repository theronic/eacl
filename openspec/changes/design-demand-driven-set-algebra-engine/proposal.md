...
- Persist only authoritative permission identity and the canonical versioned payload. Do not persist redundant expression-format, content-digest, policy-digest, admission-limit, or derived-metric attributes. Recompute structural expression/DAG metrics from the payload under each client's checked admission limits, cache them within that client by schema generation, authoritative payload identity, and effective limits, and expose an explicit cache refresh without adding derived datoms to Datomic or another built-in store.
- Maintain relationship-cardinality and physical-I/O estimates only as advisory, high-watermark-scoped cache observations. Real authorization reads update them organically without widening demand; optional adapter telemetry such as Datomic I/O stats may enrich physical-cost observations but cannot serve as cardinality authority or change a public generator/order/cursor lineage.
...
- Add a fail-closed released-v7-to-v8 permission-schema upgrade. It preflights the complete replacement under the invoking client's admission limits, installs only the canonical payload and coordination-required additive schema attributes, atomically retires old flat permission rows and writes canonical expression rows, and reuses released v7 relationship tuples without scanning, rewriting, or rebuilding them. An invalid or unwritable replacement leaves the old permission schema usable. Superseded unreleased-v8 development databases remain disposable and may be recreated.
...
- `schema-write-safety`: Validate, bound, stratify, atomically store, and safely upgrade released v7 permission schemas while preserving relationship storage and keeping derived metrics out of durable permission data.
...
- `dependency-validated-authorization-cache`: Include every positive and negative branch in proof scopes even when witnesses or short-circuiting avoid runtime reads, and scope advisory count observations by relation high-watermarks.
...
Affected code includes the SpiceDB parser and schema model, all built-in schema stores, the Datomic released-v7 upgrade path, the v8 adapter SPI, schema-generation and observation caches, sealed-plan compiler, least-path and recursive engines, scan/probe cache seams, cursor/checkpoint codecs, permission-tree rendering, counters and limits, Dafny/TLA+/generated CLJ/CLJS assurance boundary, module conformance suites, Datahike tuple-index kernels, the Datahike demo's MinIO benchmark harness, documentation, and the sibling `eacl-spicedb` differential repository. No new production dependency or persistent statistics catalog is introduced.
