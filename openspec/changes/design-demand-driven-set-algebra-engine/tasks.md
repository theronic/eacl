...
- [x] 2.8 Prove anchor-preserving max-head k-way leapfrog/galloping intersection and monotone exclusion anti-join reproduce the generic cover/predicate sequence and logical work boundary in `SeekableSetKernels.dfy`; reject repeated binary filtering as the n-ary implementation.
...
- [ ] 3.1 Change the grammar to union-before-intersection-before-exclusion precedence, left-fold repeated exclusion, and preserve explicit grouping in the source AST; retain every existing unsupported construct rejection.
