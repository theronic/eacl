...
- Add **range answer reuse**: a completed page retains each result's cursor edge, so a shorter page of the same query from the same start boundary is served as a prefix of the longer resident page (and a longer one as the resident page plus its continuation) instead of being recomputed and stored separately; derived pages are rendered and published through the ordinary path and bounded candidate-window routes are excluded.
...
- `range-answer-reuse`: Serving a shorter page as the prefix of a longer completed page of the same query and start boundary, and a longer page as a completed page plus its continuation, with retained per-result edges and identical public content.
