...
| Point checks and counts keep operation-appropriate plans | `BidirectionalReachability.dfy`, `EaclBidirectionalReachability.dfy`, `ExactCountComposition.dfy`, `MembershipProbeCheck.dfy` (probe answer = reverse-denotation membership), `BidirectionalArrowIntersection.dfy` (two-layer arm answer strategy-independent; interleaved decision bounded by the smaller side) **proved-model**; `eacl.engine.point-check-test` (probe vs enumeration oracle on six frozen baselines, O(intermediates) property, min-side cost property, randomized bidirectional differential) and `eacl.datomic.raw-plan-isolation-test` (filtered/speculative views never share sealed plans with the plain database) **checked-executable**; counting admitted work **refuted**. | Design-closed | The point-check route is the membership-probe search with bidirectional two-layer arms (`membership-probe-point-check`, `bidirectional-arrow-point-check`); the enumeration form is retained as its oracle; any optimized count route needs an independent denotation-equivalence proof. |
| Acyclic plans paginate by least-derivation-path keyset order | `LeastPathOrder.dfy` (strict total history-free order; earlier-alternative decomposition), `LeastPathEnumeration.dfy` (filtered DFS = reachable denotation once per entity; prune soundness; merge lemmas), `LeastPathResume.dfy` (seek-past-boundary = suffix; descending agreement) **proved-model**; `eacl.engine.least-path-test` (naive-oracle order+coordinate differential, reducer set-equality, every-boundary resume, descending agreement, work-bounded stream opens) and `eacl.datomic.least-path-client-test` (flat cache-off pagination through the public client; :last/:before under demand) **checked-executable**. | Design-closed | Order ABI v2 (`acyclic-keyset-pagination`): acyclic roots seal `:least-path`, recursive roots keep first-discovery order, checkpoints, and replay; the mode and `:recursive?` are folded into the plan fingerprint. |
