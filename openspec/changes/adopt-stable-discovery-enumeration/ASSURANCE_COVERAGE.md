...
| Point checks and counts keep operation-appropriate plans | `BidirectionalReachability.dfy`, `EaclBidirectionalReachability.dfy`, `ExactCountComposition.dfy` **proved-model**; counting admitted work **refuted**. | Design-closed | Build the goal-anchored point-check and exhaustion-count routes (they do not exist in the prototype); any optimized count route needs an independent denotation-equivalence proof. |
