...
- [ ] 3.4 Add mutation controls: logical-release-width not one, dynamic ordering input, eager chunk admission, integrate-on-completion, incomplete failure integration, stale-basis continuation, checkpoint regression, missing lookahead segment, entity-only interior admission key, producing-edge root key. Engine-side controls landed and killed (`eacl.engine.stable-discovery-gate-test`): eager whole-chunk release on the overlap geometry, host push/bucket order, fetched-end resume, entity-only interior key, per-rule root key. Continuation controls landed and killed in `eacl.engine.stable-page-test`: stale-basis rejection, checkpoint nonregression, missing lookahead segment. Failure-integration controls land with section 7; integrate-on-completion is parked with the concurrency change.
