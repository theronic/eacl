...
The phase must create a permanent storage boundary for sparse future qualifiers without carrying v7 compatibility through every read. It must also correct the version terminology: v8 is the library release line and both permission and Relationship storage use version 8.
...
After client construction succeeds, all authorization, lookup, count, Relationship-read, deletion, proof, and integrity paths use only v8 attributes. Production source contains no “try v8, then v7” branch and no merged cursor frontier.
...
- a mixed legacy/v8 current database;
- an in-progress or failed migration marker;
- a storage stamp other than 9 for an initialized store;
- missing or physically incompatible v8 attribute definitions.

Client construction does **not** re-enumerate the v8 graph, recompute pair parity, or rerun migration verification. The complete migration/fresh-bootstrap stamp is the serving boundary; ordinary integrity tooling and the existing supported-writer/content-proof contracts cover later corruption. A genuinely empty fresh database may install and stamp v8 during the normal explicit schema/bootstrap operation. An existing stamped v7 database must use migration even when its current Relationship count is zero, so operational intent is explicit and repeatable.
...
```clojure
(eacl.datomic.migrations.relationships-v7-to-v8/migrate! conn options)
```
...
1. verifies that the source is v7 or a recognized interrupted v7-to-v8 run; a v6 entity source is rejected with the exact existing v6-to-v7 prerequisite rather than guessed through;
...
Conversion processes bounded canonical batches. For each healthy v7 pair, one transaction adds the exact v8 pair with `nil` qualifier, retracts the exact v7 pair, advances every affected Relation version once, and records progress. If a matching v8 pair already exists from an interrupted transaction boundary, the rerun validates it and only removes the surviving source pair. A conflicting v8 value, dangling half, missing Relation, or changed endpoint identity stops the migration with diagnostics; it is never guessed through.

Finalization performs a complete source-empty check, v8 pair/uniqueness verification, target count/digest comparison against the preflight source certificate, affected-Relation version verification, and migration-marker CAS. Only that transaction writes `:eacl/storage-version 8` and `:complete`. A crash before it leaves the store unbootable but safely resumable.
...
This maintenance requirement is security-relevant: an old-format write after its range was converted could otherwise be omitted from v8; an omitted subtracting or deny Relationship may broaden permission. Detection or a changed source proof aborts rather than completing with a potentially partial graph.
...
Benchmarks compare the released four-slot baseline with five-slot `nil` v8 storage for:
...
- **Old historical artifacts are unavailable to v8 code** → retain the old deployment/database for audit or restore; do not add old-layout routing to current authorization.

## Migration Plan

1. Release migration tooling and documentation before requiring storage 8 in a serving client.
...
4. Run the backend `v7-to-v8/migrate!` function with bounded batch and progress options.
5. Require its final verification report and storage stamp 8; independently sample known grants, exclusions, arrows, and Relationship pages.
6. Deploy/restart EACL v8 clients that require storage ABI 8, discard old caches, and restart old cursors.
7. Resume writes only after all peers report target compatibility.
8. Rollback means stopping the new deployment and restoring/switching to the pre-migration database with the matching old EACL version. There is no in-place v8-to-v7 downgrade.
