...
The external-ID ↔ internal-eid boundary of the Datomic client: one coherent contract for unknown IDs, opt-in tempids, and validated client configuration. Covers `make-client`, `spice-object->internal`, `spiceomic-read-relationships`, `spiceomic-write-relationships!`, and `impl/tx-relationship`.
