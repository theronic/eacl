...
misspelled nested expiry such as `:valid-until-mss`, or an update missing its
resource, could therefore reach a remote writer. If that writer ignored unknown
fields or supplied defaults, it could persist a grant without the intended
deadline or for the wrong scope.
...
single preparation and speculative planning paths. The Dafny model now
requires every mandatory nested field as well as rejecting unknown fields,
represents malformed contents separately from a valid sequential batch, and an
executed mutation proves both production guards are observable.
