...
| Scan-response reuse and range answer derivation | `eacl.engine.scan-cache/serve`, `extend-entry`, and `caching-fetch-fn` under `eacl.engine.v8/routed-fetch-fn` (request-local memo always; the client-owned shared tier only for the request's selected adapter, scoped by the scanned relation's generation from an already resolved proof frame); `eacl.client.range-reuse/walk-key`, `window`, `lookup!` (any window inside a retained segment, or the segment's tail plus a continuation request), `compose`, and `publish!` (segment merging and per-walk caps) on least-path and first-discovery pages; `eacl.engine.v8/checkpoint-key` keyed by walk and page series, with a continuation naming the series it resumes (`:checkpoint-size`) when it leaves a retained segment | every routed physical read of the stable reducer, least-path evaluator, and membership-probe route; every page window served from retained segments or composed with one continuation; every recursive continuation across page sizes (`ScanResponseCache.dfy`, `RangeAnswerReuse.dfy`, `PaginationComposition.dfy`, `RuntimeCheckpointComposition.dfy`, the scan-response-cache bridge, and ten executed mutation controls) |
