...
(def compiler-plan-compatibility
  ...
  {:identity-version 1
   ...
   {:plan-version operator-plan/plan-version
    ...
    ;; Recursive plans whose recursion lies inside union-only operands are
    ;; generated by the delegated operand's own union plan, or else by the
    ;; flattened generator over the delegated operands' union plans.
    :recursive-generator :flattened-union-generator-v1
    :versions
...
(defn- stable-cover-plan
  ...
  A recursive plan whose recursion lies inside union-only operands has one
  of two generators:
  ...
  - Otherwise, such as under a union root, the flattened generator
    (`operator-cover-plan/seal-generator`): one synthetic union node per
    operator permission reached, over the delegated operands' union plans.
...
(defn- delegated-operand-oracle
  ...
  attempt counter of its routed read path. A batched oracle also returns
  `:holdings`, `stable-route/subject-holdings` over the request's membership
  context, so relation leaves decide many candidates of one subject from one
  scan."
  [db mode]
  ...
  (let [cover-plan (stable-cover-plan db plan)
    ...
    (if (nil? anchor-eid)
      ...
      (let [external-accept? (:accept? candidate-filter)
            ...
            oracle (when (operator-plan/delegated-permissions plan)
          ...
          (run-routed
           (fn []
             (let [scope-identity (operator-snapshot-proof-identity db)]
               (if (operator-recursive/recursive-plan? plan)
                 (let [oracle (when (operator-plan/delegated-permissions plan)
                                (delegated-operand-oracle db :point))
                       decision
