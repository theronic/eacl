...
(def compiler-plan-compatibility
  ...
  {:identity-version 1
   ...
   {:plan-version operator-plan/plan-version
    ...
    ;; generated by the delegated operand's own union plan.
    :recursive-generator :delegated-union-plan-v1
    :versions
...
(defn- stable-cover-plan
  ...
  A recursive plan whose generator chain ends at a delegated union-only
  permission is generated by that permission's own sealed union plan: the
  stream the union engine produces for a lookup of the operand itself, with
  no synthetic operator nodes between the account and its emission. Its
  fingerprint is the cover fingerprint every recursive operator cursor
  authenticates, so a cursor minted under another generator is rejected."
  [db plan]
