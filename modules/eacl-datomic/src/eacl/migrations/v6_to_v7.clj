...
(defn detect-storage-version
  ...
  (let [v6? (boolean (seq (d/datoms db :aevt v6-relation-name-attr)))
        v7? (boolean (seq (d/datoms db :aevt relationship-storage/forward-attribute)))]
    (cond
      (and v6? v7?) :mixed
      v7?           :v7
      v6?           :v6
...
(defn verify-backfill
  ...
  (let [missing-sample (into [] (take 10) (missing-tuples db))]
    {:v6-relationship-entities (count-datoms db v6-relation-name-attr)
     :v7-forward-tuples        (count-datoms db relationship-storage/forward-attribute)
     :v7-reverse-tuples        (count-datoms db relationship-storage/reverse-attribute)
...
(defn- stamp-storage-version!
  "Records the completed forward storage migration on the EACL singleton."
  [conn]
